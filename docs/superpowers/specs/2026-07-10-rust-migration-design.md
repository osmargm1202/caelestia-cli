...
**Se porta (12 → 10 subcomandos nativos):** shell, toggle, scheme, screenshot, record, clipboard, emoji, wallpaper, resizer, search.
...
2. **Util core + subcomandos triviales** — `util/` (paths, io, notify), `ipc/hypr.rs`, `colour.rs`. Nativos: toggle, shell, clipboard, emoji, screenshot, record, search.
...
6. **Limpieza** — stubs install/update, borrar `python-ref/` y delegación, README con lista de binarios runtime (grim, slurp, swappy, wl-clipboard, cliphist, fuzzel, gpu-screen-recorder, ffmpeg, dart-sass, libnotify, dconf, killall, xdg-utils, curl, qs/caelestia-shell), flake final sin Python.
